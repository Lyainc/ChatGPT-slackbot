...
def handle_dm(event, say):
    ...
    try:
        ...
        debug_log(f"Received message: {user_message} from user: {user_name} (ID: {user_id}) in DM: {channel_id}")
        
        if user_message.startswith("//대화시작"):
            question = user_message.replace("//대화시작", "").strip()
            
            debug_log(f"Extracted question: {question} from user: {user_name} (ID: {user_id})")
            ...
            debug_log(f"User conversation updated for user: {user_name} (ID: {user_id}): {user_conversations[user_id]}")
            say(text="질문을 인식했습니다. 대화를 시작합니다.", thread_ts=thread_ts)
            
            # 시간 측정 시작
            start_time = time.time()
            say(text="새로운 질문을 인식했습니다.", thread_ts=thread_ts)
            ...
            if "thread_ts" in event:
                ...
                logging.info(f"Ended conversation for user: {user_name} (ID: {user_id})")
            else:
            ...
            if user_id in user_conversations:
                ...
                # 답변과 시간 정보를 슬랙에 출력
                say(text=f"{answer}\n응답 시간: {elapsed_time_ms:.2f} ms", thread_ts=thread_ts)
                debug_log(f"Added user message to conversation for user: {user_name} (ID: {user_id}): {user_message}")
